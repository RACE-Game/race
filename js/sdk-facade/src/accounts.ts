...
import { CheckpointOnChain, ENTRY_LOCKS, Fields, Indices, RECIPIENT_SLOT_TYPES, VOTE_TYPES } from '@race-foundation/sdk-core'
...
export abstract class EntryType implements IEntryTypeKind {
  ...
  generalize(): RaceCore.EntryType {
    if (this instanceof EntryTypeCash) {
      return {
        kind: 'cash', minDeposit: this.minDeposit, maxDeposit: this.maxDeposit
      }
    } else if (this instanceof EntryTypeTicket) {
      return {
        kind: 'ticket', amount: this.amount
      }
    } else if (this instanceof EntryTypeGating) {
      return {
        kind: 'gating', collection: this.collection
      }
    } else {
      return {
        kind: 'disabled'
