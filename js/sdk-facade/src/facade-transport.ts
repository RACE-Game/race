...
export class FacadeTransport implements ITransport {
  ...
  async createGameAccount(wallet: IWallet, params: CreateGameAccountParams, response: ResponseHandle<CreateGameResponse, CreateGameError>): Promise<void> {
  ...
  async join(wallet: IWallet, params: JoinParams, response: ResponseHandle<JoinResponse, JoinError>): Promise<void> {
    ...
    if (params.createProfileIfNeeded) {
      const createPlayerProfileIx: CreatePlayerProfileInstruction = { playerAddr, nick: wallet.walletAddr.substring(0, 6), pfp: undefined }
