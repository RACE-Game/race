...
use crate::js::{rget, rget_string, rget_u16, rget_u64};
...
impl AppHelper {
    ...
    #[wasm_bindgen]
    pub async fn create_profile(&self, wallet: JsValue, nick: &str, pfp: &str) -> Result<()> {
        let pfp = if pfp.eq("") {
            None
        } else {
            Some(pfp.to_owned())
    ...
    #[wasm_bindgen]
    pub async fn list_games(&self, registration_addrs: Box<[JsValue]>) -> Result<Array> {
        let games = Array::new();
